...
let anchor_to_msgpack = function
  | Absolute { row; col } ->
      ...
      let pairs =
        if row <> 0 then pairs @ [ (P.Str "row", P.Int row) ] else pairs
      in
      let pairs =
        if col <> 0 then pairs @ [ (P.Str "col", P.Int col) ] else pairs
      in
      P.Map pairs
  | Tmux_pane { pane_id; row; col } ->
      let pairs = [ (P.Str "type", P.Str "tmux_pane") ] in
      let pairs =
        if pane_id <> "" then pairs @ [ (P.Str "pane_id", P.Str pane_id) ]
        else pairs
      in
      let pairs =
        if row <> 0 then pairs @ [ (P.Str "row", P.Int row) ] else pairs
  ...
  | Nvim_win { win_id; pane_id; buf_line; col } ->
      ...
      in
      let pairs =
        if win_id <> 0 then pairs @ [ (P.Str "win_id", P.Int win_id) ]
        else pairs
      in
      let pairs =
        if buf_line <> 0 then pairs @ [ (P.Str "buf_line", P.Int buf_line) ]
        else pairs
      in
      let pairs =
        if col <> 0 then pairs @ [ (P.Str "col", P.Int col) ] else pairs
...
(** Build a map from string-value pairs, omitting entries where value is
    zero/empty. *)
let map_of_pairs pairs = P.Map (List.map (fun (k, v) -> (P.Str k, v)) pairs)
  ...
  if try_connect () then ()
    ...
    let rec wait n =
      if n <= 0 then failwith "timed out waiting for kgd to start"
        ...
        Unix.sleepf 0.1;
        if try_connect () then () else wait (n - 1)
      end
    in
    wait 50
  end
...
let send_raw client data =
  ...
  Fun.protect
    ~finally:(fun () -> Mutex.unlock client.write_lock)
    (fun () ->
...
let send_request client msg_id method_name params =
  let params_array =
    match params with P.Nil -> P.Array [] | p -> P.Array [ p ]
  in
  let msg =
    P.Array
      [ P.Int _MSG_REQUEST; P.Int msg_id; P.Str method_name; params_array ]
  in
...
let send_notification client method_name params =
  ...
  in
  let msg =
    P.Array [ P.Int _MSG_NOTIFICATION; P.Str method_name; params_array ]
  in
  send_raw client (P.encode msg)
...
let handle_notification client msg =
  ...
  | [ _; method_v; params_arr_v ] -> (
      ...
      let params = match params_list with p :: _ -> p | [] -> P.Nil in
      ...
      | "theme_changed" -> (
          ...
          | Some cb ->
              ...
              let fg =
                match fg_v with
                | P.Nil -> color_zero
                | _ -> color_of_msgpack fg_v
              in
              let bg =
                match bg_v with
                | P.Nil -> color_zero
                | _ -> color_of_msgpack bg_v
              in
              cb fg bg
          | None -> ())
      | _ -> ())
...
let connect ?(socket_path = "") ?(session_id = "") ?(client_type = "")
    ?(label = "") ?(auto_launch = true) () =
  let path =
    resolve_socket_path (if socket_path = "" then None else Some socket_path)
  in
  ...
  let hello_params =
    ...
    let pairs =
      if session_id <> "" then
        pairs @ [ (P.Str "session_id", P.Str session_id) ]
      else pairs
    in
    P.Map pairs
  in
...
let set_on_visibility_changed client cb =
  client.on_visibility_changed <- Some cb
...
let place client handle ~anchor ~width ~height
    ?(opts = { src_x = 0; src_y = 0; src_w = 0; src_h = 0; z_index = 0 }) () =
  let pairs =
    [
      ("handle", P.Int handle);
  ...
  in
  let pairs =
    if opts.src_x <> 0 then pairs @ [ ("src_x", P.Int opts.src_x) ] else pairs
  in
  let pairs =
    if opts.src_y <> 0 then pairs @ [ ("src_y", P.Int opts.src_y) ] else pairs
  in
  let pairs =
    if opts.src_w <> 0 then pairs @ [ ("src_w", P.Int opts.src_w) ] else pairs
  in
  let pairs =
    if opts.src_h <> 0 then pairs @ [ ("src_h", P.Int opts.src_h) ] else pairs
  in
  let pairs =
    if opts.z_index <> 0 then pairs @ [ ("z_index", P.Int opts.z_index) ]
    else pairs
  in
...
let unplace_all client = notify client "unplace_all" P.Nil

let free client handle =
  let params = map_of_pairs [ ("handle", P.Int handle) ] in
  ignore (call client "free" params ())

let register_win client ~win_id ?(pane_id = "") ~top ~left ~width ~height
    ~scroll_top () =
  let params =
    map_of_pairs
      [
...
let update_scroll client ~win_id ~scroll_top =
  let params =
    map_of_pairs [ ("win_id", P.Int win_id); ("scroll_top", P.Int scroll_top) ]
  in
...
let stop client = notify client "stop" P.Nil
