...
let encode_int buf n =
  if n >= 0 then begin
    if n <= 0x7f then
      ...
      buf_add_uint8 buf n
    end else if n <= 0xffff then begin
      (* uint 16 *)
    ...
    if n >= -32 then
      ...
      buf_add_uint8 buf (n land 0xff)
    end else if n >= -32768 then begin
      (* int 16 *)
...
let get_uint16_be buf ofs =
  if ofs + 1 >= Bytes.length buf then failwith "msgpack: unexpected end of input";
  (Char.code (Bytes.get buf ofs) lsl 8) lor Char.code (Bytes.get buf (ofs + 1))

let get_uint32_be buf ofs =
  if ofs + 3 >= Bytes.length buf then failwith "msgpack: unexpected end of input";
...
let get_bytes buf ofs len =
  if ofs + len > Bytes.length buf then failwith "msgpack: unexpected end of input";
...
let rec decode buf ofs =
  ...
  if tag <= 0x7f then
    ...
    (Str s, ofs + len)
  end else if tag >= 0x90 && tag <= 0x9f then begin
    (* fixarray *)
    let len = tag land 0x0f in
    decode_array buf ofs len
  end else if tag >= 0x80 && tag <= 0x8f then begin
    (* fixmap *)
    let len = tag land 0x0f in
