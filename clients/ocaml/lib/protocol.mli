(** Minimal msgpack encoder/decoder for kgd msgpack-RPC.
    ...
    Handles the subset of msgpack needed by the kgd protocol: fixint,
    uint8/16/32, int8/16/32, nil, bool, bin8/16/32, str8/16/32, fixstr,
...
(** [decode buf ofs] decodes one msgpack value starting at offset [ofs] in
    [buf]. Returns [(value, next_offset)].
    @raise Failure if the buffer is too short or the format byte is unsupported.
*)

val decode_all : bytes -> t list
(** [decode_all buf] decodes all msgpack values from [buf]. Returns a list of
    decoded values.
...
(** [to_list v] extracts an array as a list, returning [[]] for non-array
    values. *)
