...
let test_encode_decode_fixmap () =
  let m = P.Map [
    (P.Str "name", P.Str "test");
    (P.Str "value", P.Int 42);
  ] in
...
let test_response_format () =
  ...
  let msg = P.Array [
    ...
    P.Int 1;
    P.Nil;
    P.Map [ (P.Str "client_id", P.Str "abc-123") ];
  ] in
  let encoded = P.encode msg in
  let (decoded, _) = P.decode encoded 0 in
...
let test_anchor_nvim_win () =
  let open Kgd_client in
  let _anchor = Nvim_win { win_id = 1000; pane_id = "%1"; buf_line = 42; col = 5 } in
...
let test_placement_info () =
  let open Kgd_client in
  let p = { placement_id = 1; client_id = "abc"; handle = 42;
            visible = true; row = 5; col = 10 } in
  check "placement_info fields" (p.placement_id = 1 && p.handle = 42 && p.visible)
